		...
		try {
			...
			if (tokens.front().type == VARIABLE) {
				...
				if (!checkVariable(variable)) {
					cout << "This variable already exists! \n";
					throw 404;
				}
				if (tokens.front().type == ASSIGN_DEC || tokens.front().type == ASSIGN_INT) {
	...
	void printVariable(Token current) {
		try {
			...
			if (tokens.front().type == VARIABLE) {
				...
				for (int i = 0; i < integer.size(); i++) {
					if (variable.value == integer[i].second) cout << "Output: " << integer[i].first << "\n";
				}
				for (int i = 0; i < decimal.size(); i++) {
					if (variable.value == decimal[i].second) cout << "Output: " << decimal[i].first << "\n";
				}
